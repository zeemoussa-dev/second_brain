...
export function EmailsPage() {
  ...
  // The newest thread opens by itself: a reader pane that starts empty wastes
  // half the screen, and the first thing wanted is almost always the last email.
  useEffect(() => {
    if (!items || items.length === 0 || selected) return;
    const next = new URLSearchParams(searchParams);
    next.set('thread', items[items.length - 1].stem);
    setSearchParams(next, { replace: true });
  }, [items, selected, searchParams, setSearchParams]);
