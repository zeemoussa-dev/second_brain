...
def test_listing_returns_the_shipped_blueprints() -> None:
    ...
    assert "librarian" in [b["id"] for b in response.json()]


def test_preflight_creates_nothing_and_reports_peers() -> None:
    response = client.get("/blueprints/librarian/preflight")
    body = response.json()

    assert response.status_code == 200
    assert body["ok"] is True
    assert set(body["peers"]) == {"files-manager", "notes-manager"}
    assert body["templates"] == ["file", "note", "research-kb-doc", "thread"]
...
def test_a_blueprint_that_cannot_install_is_409_not_500(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda self, bid, wire_peers=True: {"installed": False, "problems": ["Skill 'x' is not in the catalog"]},
    )

    response = client.post("/blueprints/librarian/install")
...
def test_an_existing_but_unsatisfiable_blueprint_is_200_not_404(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda self, bid: {"blueprint_id": bid, "ok": False, "problems": ["template 'file' missing"]},
    )
...
def test_peers_can_be_installed_without_touching_primary(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        BlueprintManager, "install",
        lambda self, bid, wire_peers=True: seen.setdefault("wire_peers", wire_peers) or {"installed": True},
    )

    client.post("/blueprints/librarian/install?wire_peers=false")
