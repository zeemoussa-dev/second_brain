...
# real vault path -- Hermes Skills have no knowledge of this backend's
# config and will keep reading/writing Templates at
# <vault>/.second-brain/data/Templates/ regardless of anything below.
# THIS copy drops the ".second-brain" prefix because its one real caller
# (business/cockpit/documents.py) now passes settings.second_brain_data_path
# directly (the app-database root itself, independent of vault_path since
# this same change) rather than settings.vault_path -- if this file is
# ever re-copied from the canonical source to pick up an engine fix,
# RE-APPLY this line, don't silently overwrite it back.
