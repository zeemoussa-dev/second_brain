...
- **Root cause:** Hermes runs a profile's cron jobs only from that profile's own gateway. `meeting-capture-recurring` lived in `meeting-prep-agent`, whose gateway was stopped, so it last ran 2026-09-01 and its `next_run_at` stayed at 2026-09-01 21:24. Whether My Day's Refresh -- which fires the same job with `hermes cron run` -- ever ran it is unconfirmed: no execution is recorded for the stranded job, and no one-off meeting note was written after 2026-09-01. (Corrected 2026-09-22: this entry first claimed `cron run` only queues for the next tick; it actually executes the job inline, see the trap below.) The Hermes health checks report "Scheduled jobs configured" and the Pipeline showed a `cron_next_run_at` three weeks in the past, and nothing flagged either.
...
- **Trap found while fixing it:** `hermes cron run <job>` executes the job INLINE in the calling process (`source=direct` in `hermes cron runs`), not on the scheduler. Killing that process -- a shell timeout did it here -- leaves the execution `unknown`, and further manual runs are refused ("already being fired by the scheduler") until the scheduler's own next tick. Never wrap `cron run` in a timeout.
...
- **Also (2026-09-26):** it is a REGRESSION on a plugin screen. The CBO install's Strategic Entities plugin carried its own markdown renderer until `BUG-078` was fixed, and that renderer drew tables and translated `> [!abstract]` callouts. Adopting `NoteText` was right -- it brought mermaid, real wikilinks and one rendering for one note -- but the shared renderer draws LESS than the copy it replaced, so that screen went backwards: measured on `/strategic-entities/ADNOC`, 0 `<table>` elements, 7 raw pipe blocks, `[!abstract]`/`[!info]`/`[!success]` as text. Every plugin that adopts the host contract inherits this, which is the argument for fixing it in the contract rather than in each screen.
...
- **Fix:** one exported markdown pipeline, `NOTE_MARKDOWN_PLUGINS`, used by the note view, `NoteText` (Cockpit summaries and every plugin screen) and chat (`ADR-031`) -- `remark-gfm` passed once rather than three times, because a list each caller assembles is how the surfaces drifted apart. Callouts are a remark transform of our own (`remarkCallouts.ts`): the off-the-shelf plugin replaces the first paragraph with a raw `html` node, which react-markdown drops without `rehype-raw`, so the callout's own first line would have vanished and `ADR-050`'s no-raw-HTML rule with it. It marks the blockquote `callout callout-<type>` with a title paragraph, styled for every surface in `styles/markdown.css`; the note view's and chat's own quote rules say `:not(.callout)` instead of painting over it. Verified in the browser on this install: `/browse/MACC and Marketplace` now has a real `<table>` and 2 callouts with their accents, 0 raw pipe runs and 0 `[!` markers; through `NoteText` itself -- the plugin-screen half -- a table, both callouts with their text intact, task-list checkboxes, strikethrough and an autolink. `remark-breaks` is deliberately left out: it changes how every existing note renders and wants deciding on its own. The contract is unchanged, so `FRAMEWORK_API` stays at 6 and no plugin is republished -- pulling the framework is enough. `e094658`.
...
- **Fix:** `list_documents` walks the attachment folder through `vault_manager.long_path()` -- the glob, the `stat()` used for sorting, `read_note` and the `iterdir()` for the real file -- and hands back the ordinary path form, so what leaves the module still compares against `vault_path`. A note that still cannot be stat'd sorts last instead of raising; one that cannot be read is skipped with the reason logged. Separately, a panel can no longer take the view down with it: `build_cockpit_view` reads `people` and `related_documents` through a wrapper that logs and returns empty, so the worst a broken panel does is come back empty (the original failure's shape, not just its cause). Four tests, including a real attachment note past 260 characters; three of them fail on the previous code. Not reproducible on the PSS install (0 such subjects there), so the CBO install is the real check -- it reported 171. `79a73e5`.
...
- **Severity:** Major
- **Status:** Fixed
- **Found:** 2026-09-22, My Day listing four of the five meetings captured for the day.
- **Root cause:** the index is a dict keyed by filename stem (`VaultManager.get_index()`, and the plugin API's `vault.index()` promises exactly that). Obsidian allows the same name in different folders, so the last note walked wins and the rest vanish from every index reader. A meeting's invitation email is captured as a thread message named `<date>-<subject>` -- the meeting note's own name -- so it hides the meeting.
- **Repro:** a meeting note and a thread message both named `2026-09-22-Masdar-Core42.md`; open My Day.
- **Expected:** both notes are indexed; My Day lists the meeting.
- **Actual:** the meeting is missing from My Day. On the reporting install 100 notes across 19 names share a name, and 3 meetings are hidden this way (two from 2026-09-18 and one from 2026-09-22).
- **Note:** keying the index by path, or by stem with collisions kept, changes the plugin API contract (`vault.index()` is documented as keyed by stem), so it needs a `FRAMEWORK_API` decision, not a patch.
- **Fix:** the rebuild keeps a list of every note beside the by-name map (`ADR-028`). Counting, listing, tags, graph nodes and search read the list; `get_index()` stays a map keyed by stem, because the stem is how a wikilink and a Cockpit URL address a note -- re-keying it by path would have broken all of those. Which note wins a shared name is now a rule instead of walk order: the note whose folder is named after it (the subject note), then the shallower path, then alphabetically. The Plugin API gains `vault.entries()` and `FRAMEWORK_API` moves 4 -> 5, so installed plugins are gated rather than silently reading the deduped map; `my-day` 1.5.0 and `entities` 1.0.3 sweep it and are installed here. Verified on this install: the 2026-09-22 Masdar-Core42 meeting, previously hidden behind its own invitation email, is listed again. **Not fixed:** semantic search, whose vector store is keyed by stem on disk -- fixing it means re-embedding the vault and is its own change. `dd2978f`.
