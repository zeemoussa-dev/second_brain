...
- **[2026-09-12] A company is discovered two ways, and only one of them has a domain.** Mail from a company yields a domain; a company merely discussed in a thread yields only a name, and any discovery pass built on domains is blind to it. Put unresolved names in front of the operator as inert rows (`Ignore: Yes`, `Created: No`, empty `Domain`) with the threads they came from -- a bare name cannot be judged, and a row that creates something is not a question.
...
- **[2026-09-12] Match a company on an identifying form of its name, not the string a reader typed.** A reader quoting a contract wrote "Mubadala Health LLC", which matched no hub, while the same reader's plain "Mubadala" matched the PARENT -- so the affiliate's thread was filed under the parent. Compare lowercased, punctuation dropped, dotted initialisms collapsed and the trailing legal form removed; match the hub's own spelling first, and never let stripping fold an affiliate into its parent.

- **[2026-09-12] A Skill may be deployed to a DIFFERENT profile; resolve a sibling Skill by searching profiles too.** Tagging's attachment step reported "not found" every night because summarize-and-tag-files sits under `files-manager` while the pass runs under `email-capture`. The fix is the lookup, never a second deployed copy -- duplicated scripts are what once left 228 stale copies across 41 profiles.

- **[2026-09-12] An identifier an agent has to retype WILL arrive truncated; resolve it by uniqueness, and refuse ambiguity.** 44 of 52 Enrichment failures were a unique TAIL of an 80-character Thread id, 8 matched nothing, none were ambiguous. Accept a fragment only while it can mean one thing: writing onto the wrong note is worse than failing loudly.

- **[2026-09-12] On Windows, `os.replace` over a file just written fails with access-denied while the virus scanner still holds it, and a lock file being deleted gives PermissionError rather than FileExistsError.** Both killed parallel jobs (two of five runs). Give every writer its own scratch name, retry the rename, and treat access-denied on a lock as "held", not fatal.
...
- **[2026-09-21] A plugin is installed from wherever it lives, and the three install-time gates have exactly one implementation.** An install takes a git repository at a branch, tag or commit, or a folder, and never requires a package committed into this framework's tree (`ADR-024`) - which is what lets an agent install run a plugin it wrote without touching the framework repo (`ADR-023`). The gates that run at install time are the `framework_api` exact match, the import boundary, and that the screens build here; the plugin's own tests and the immutable-version rule stay with publishing, because installing must not need a plugin's test dependencies and a tracked branch moves while its version string does not. Those gates live in `business/core/plugins/{import_rules,screen_build}.py` and are loaded by file path from `scripts/`, so publishing still boots no application while there is one copy of each rule. An installed plugin records the source it came from, and **Update** means "pull that source again" - so a plugin installed from the Marketplace has nothing to pull and says so.

- **[2026-09-25] Iterating the vault means `get_entries()` / `api.vault.entries()`; `get_index()` is lookup-by-name only.** Two notes may legitimately carry the same file name (Obsidian allows it, and capture produces it: a meeting and the invitation email captured under the meeting's own name), so the by-name map holds ONE of them -- anything that counts, lists, filters or sweeps through that map silently drops the rest. Which note wins a shared name is a rule, not walk order: the note whose parent folder is named after it, then the shallower path, then alphabetically (`_is_preferred`). `BUG-076`, `ADR-028`. Semantic search still keys its on-disk store by stem and is NOT covered by this.
