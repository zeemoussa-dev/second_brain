...
- **`src/backend/app/vault/vault_manager.py` is an OLDER engine than the canonical copy under `Hermes-Provisioning/shared/`.** It has no `create_dynamic_child()`, no `_section_allowed_callers` and no `data_root()`. Never assume a capability documented for the engine exists in the backend's copy - check the file you are actually importing.
