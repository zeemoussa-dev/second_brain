...
- `[REQ-SB-77-US-01-AC-05]` No company at all: real Person note `<operator-email>` (a personal `live.com` domain sender — `derive_company_from_email` correctly returns `None`; tags carry no `company/` entry at all). Ran the relink: outcome `company: None`, no tag, no link. Byte-for-byte diff against the pre-test snapshot confirmed ZERO change to this note at all (0-line `Compare-Object` diff). **PASS.**
