...
## 2026-09-05 — Harvested from `MEMORY.md` during the memory reorganisation

Not from a sprint retrospective. These were living in framework memory as build
narrative; the transferable part is below and the full original text is in
`Archive/2026-09-05/MEMORY-to-dev-record.md`, by entry number.

### Patterns (do more of this)

- **Dual-syntax embed resolver** — Take a plain `list[str]` of vault-relative paths so nothing depends on how the selection was built, scan each note body with two regexes (wikilink-embed and markdown-image-link), and resolve each target in a fixed fallback order: the referencing note's own folder first, then every ancestor up to the vault root. Relative-to-the-note is this vault's dominant real convention, so try it first rather than searching globally. _(entry 056)_
- **Genuinely-unfiltered tree read** — When a feature must show everything, walk `Path.iterdir()` directly rather than reusing `list_all_note_paths()` or the note index - both deliberately exclude OKF-reserved files and `_`-prefixed folders. Compute `child.is_dir()` **once** per entry and reuse that boolean for both the sort key and the type decision. _(entry 057)_
- **Raw-HTTP client layer** — A new provider client is `data_access/<provider>_client.py`, raw I/O only: build, send via `httpx`, parse. A dedicated `<Provider>ClientError` on every failure path including a body that does not parse - never let a `KeyError` or JSON-decode failure escape. It reads `app.config.settings` directly at call time; a Manager owns config at rest, not runtime dispatch. _(entry 059)_
- **LLM-composing decision function** — A business-layer function that reasons over given data should compose the provider client (never `httpx` directly, never another business module), build its prompt in a private helper taking plain parameters so it stays monkeypatch-testable, and parse the reply with a **strict** match against only the given candidate set - returning `None` on zero or multiple matches rather than guessing. _(entry 061)_
- **Import-only write path for a read-only Manager** — `data_access` gains a raw `write_<entity>_json()` with no validation; the Manager gains `import_<entity>()` that validates by round-tripping through its own existing read-side parser first, so a malformed shape raises before anything reaches disk, then writes and returns `get_by_id()`. Update the module docstring in the same change so a stale "read-only" claim never survives. _(entry 063)_
- **Two-phase upload, preview, decide, commit** — Keep the real `File` object in component state and re-submit the same one at commit - a two-phase backend never trusts a cached preview either. Render a decision control **per conflicting item**, never one shared control for a batch, keyed exactly as the backend's decisions map expects. Keep commit genuinely `disabled` until every decision is made. _(entry 064)_
- **Folder-tree multi-select holds files only** — The selection `Set<string>` contains file paths and never a folder's own path. A folder checkbox's state is derived at render time by walking the fetched tree; toggling it adds or removes all descendant files at once. The exposed selection is then a flat, ready-to-consume file list with no expansion step anywhere downstream. _(entry 066)_
- **Check the upstream state design before re-implementing** — When a downstream task's spec says "expand the selection before sending", read the upstream page's actual selection design first. If it already holds expanded file paths, the expansion step is satisfied by construction and the correct answer is `Array.from(selection)` and zero new code. Log it as scope-internal reconciliation, not a re-spec. _(entry 067)_
- **Per-artifact gating in an import orchestrator** — Gate on an explicit per-artifact decision the moment `conflicts: true`, returning an honest `"failed"` outcome naming the missing decision rather than defaulting silently. Dispatch each kind inside the caller's own `try/except` so one artifact's failure never aborts the loop or corrupts another's reported outcome. `keep_both` generates its alternate id from one shared scheme. _(entry 039)_
- **Monkeypatch a regression baseline when no commit exists to diff** — When a repo carries several tasks' edits as one uncommitted working-tree change, there is no "immediately before" ref to check out. Reconstruct the baseline by monkeypatching only the one dependency function the change added, then run the same sample through the real, unmodified production path. No file edits, no revert step. _(entry 151)_
- **Verify at the smallest scope that actually proves it** — A task's Tests block naming an end-to-end live trigger as "strongest verification" is not binding when triggering it would produce real content beyond the task's own scope. Running the migrated script directly against two already-processed items proved the same thing without generating ~20 threads of LLM-authored summaries the task explicitly excluded. _(entry 049)_

### Antipatterns (avoid this)

- **Citing a story's disclosed risk without re-checking it** — A risk section goes stale between being written and being built. One story's central risk - no live back-channel, stateless one-shot turns - had already been fixed by a persistent session layer the day before the story was written. Re-verify a disclosed risk against current code before designing around it. _(entry 017)_
- **Treating shipped and enabled as working** — A Vault Scope and Guardrails editing UI had been live for weeks and had never worked for any agent, broken at two stacked layers - the PATCH body model silently discarded every field but `icon` and `color`, so the frontend's writes went nowhere. Nothing errored. When a feature has no confirmed successful use, assume it has none. _(entry 130)_
- **A panel that lies about fields it cannot write** — The same audit found controls that looked functional and were not, alongside fields the backend provided that the panel never surfaced. Audit a detail panel against what the backend **actually returns** and what the write path **actually accepts**, in both directions - a rendered control is not evidence of a wired one. _(entry 141)_
- **Applying a layering rule once and calling it applied** — The Manager-does-no-raw-I/O rule was corrected into `ProviderManager` a second time, after the operator asked "shouldn't it be going through data access?" Reading entity data values out of `settings` is the same class of I/O as reading the JSON store. When a layering rule is established, sweep for the other violators in the same pass. _(entry 129)_
- **Fixing an out-of-scope bug in the task that found it** — A pre-existing defect in already-Done code surfaced while building an unrelated task. It was flagged for a `/bug` rather than fixed in place, because fixing it would have edited files outside the task's own `## Files to Modify`. Finding a bug is not authority to fix it inside someone else's scope. _(entry 037)_
