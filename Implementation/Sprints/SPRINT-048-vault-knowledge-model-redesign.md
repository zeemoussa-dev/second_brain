...
- **A throwaway scratch vault (session scratchpad, `VAULT_PATH` env-overridden) as the standard manual-verification harness.** Used consistently across `T04`/`T05`/`T06` — real function calls, real filesystem state, real assertions, and the actual configured vault (`Moussa MD\Moussa Brain`) never touched by any test run. Cheap to set up, cheap to discard, and catches real bugs a mocked/pure-unit test would miss (e.g. `T06`'s own regression check needed a real one-level-vs-multi-level glob comparison against real files on disk).
